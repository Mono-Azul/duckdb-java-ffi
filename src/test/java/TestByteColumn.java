...
public class TestByteColumn
{
    ...
    void testByteArray()
    {
        ...
        {
            Result res = con.query("SELECT NullValue, ID FROM ByteTest ORDER BY id DESC LIMIT 5;");

            ByteColumn Col = (ByteColumn)res.Columns.getFirst();
    ...
    void testByteRows()
    {
        try (Connection con = DbForTestRun.getConnection())
        {
            Result res = con.query("SELECT NullValue, ID FROM ByteTest WHERE ID >= 0 ORDER BY id LIMIT 3000;");
    ...
    void testByteNull()
    {
        try (Connection con = DbForTestRun.getConnection())
        {
            Result res = con.query("SELECT ID, NullValue FROM ByteTest WHERE ID = -1;");
