...
public class TestBooleanColumn
{
    ...
    void testBoolArray()
    {
        ...
        {
            Result res = con.query("SELECT NullValue, ID FROM BooleanTest ORDER BY id DESC LIMIT 5;");

            BooleanColumn Col = (BooleanColumn)res.Columns.getFirst();
    ...
    void testBoolRows()
    {
        try (Connection con = DbForTestRun.getConnection())
        {
            Result res = con.query("SELECT NullValue, ID FROM BooleanTest WHERE ID >= 0 ORDER BY id LIMIT 3000;");
    ...
    void testBoolNull()
    {
        try (Connection con = DbForTestRun.getConnection())
        {
            Result res = con.query("SELECT ID, NullValue FROM BooleanTest WHERE ID = -1;");
